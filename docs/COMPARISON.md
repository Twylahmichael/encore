...
1. **Brand name** — ~~held, per your instruction. No title/metadata change made. Still an open question for the client.~~ **Resolved in the fifth pass below**: client confirmed "Encore Fitness Studio."
...
## Fourth pass — real schedule, replacing the round-robin

The client confirmed the actual current class roster and coach assignments, which turned out to differ from what the live WordPress site's schedule table has published — both in class names and coaches, not just in who teaches what. Applied via `supabase/migrations/0005_real_schedule.sql`:

| Day | AM | PM |
|---|---|---|
| Monday | Aerobics — Coach Karis | Toning — Coach Ewid |
| Tuesday | CrossFit — Coach Ray | Taecombat — Coach Okeke |
| Wednesday | CrossFit — Coach Ray | CrossFit — Coach Vitalis |
| Thursday | Aerosteps — Coach Vitalis | CrossFit — Coach Ray |
| Friday | Steps — Coach Malik | Zumba — *unassigned* |
| Saturday | Boxing (9:00–10:00) — *unassigned* | — |

Notes:
- **Saturday collapsed from two sessions to one.** The live site (and this replica's earlier seed) had "Bootcamp" at 9:00–10:00 and "Yoga/Boxing" at 11:00–16:00. The client's confirmed schedule has a single "Boxing" session. I asked which time to use rather than guess — confirmed as 9:00–10:00, replacing the old Bootcamp slot; the old 11:00–16:00 slot was deleted (verified zero bookings existed against it before deleting).
- **Friday PM (Zumba) and Saturday (Boxing) are deliberately unassigned** — `coach_id` is `null`, matching the "classes missing a coach" flag the admin Dashboard already uses. This is a data fact, not a UI gap: staff fill these in weekly.
- **"Coach TBD" now renders everywhere a coach is unassigned**, instead of the line disappearing. Fixed two spots that previously went blank on a null coach: the public Fitness Studio schedule cards (`src/pages/FitnessStudio.tsx`) and the member portal's Book a Class list (`src/pages/portal/BookClass.tsx`, previously said "Coach TBA" — standardized to "Coach TBD"). The admin Schedule Manager's edit control already said "— Unassigned —", which is correct as-is for an editable field and wasn't changed.
- **`src/data/schedule.ts` (the static, verbatim-from-WordPress fallback) was deliberately left unchanged.** Its whole purpose is documenting what the live WordPress site actually publishes, and it's still used as a graceful-degradation fallback if the live Supabase fetch fails — changing its content would break both of those. This does mean that during the (usually sub-second) window before the live fetch resolves, or if it ever fails, viewers would briefly see the site's old published schedule instead of the real one; a pre-existing, acceptable tradeoff, not new to this pass.
- `supabase/seed.sql` was updated to seed this real schedule + all 6 coaches directly, so a fresh project no longer needs the now-superseded round-robin migration to get useful data.

## Fifth pass — brand name resolved

Client confirmed the real business name is **"Encore Fitness Studio"** — not "Encore Fitness and Nutrition," which is what the live site's own `<title>`/`og:site_name` metadata actually says (the original finding above, at the top of this document, is left unedited as the record of what efn.co.ke itself currently publishes — it hasn't been changed there; only this app's own text was updated).

Searched the whole codebase for "Encore Fitness and Nutrition" and for "efn"/"EFN" as a name reference (excluding URLs/file paths, which stay). Changed:

- `index.html` — `<title>` and meta description
- `src/components/Header.tsx` — logo `alt` text
- `src/components/Footer.tsx` — copyright line
- `src/pages/PrivacyPolicy.tsx` — "operated by" line
- `CLAUDE.md` — project description, with a note on the discrepancy with the live site's own metadata

**Deliberately left unchanged**, flagged rather than silently decided:
- **`docs/COMPARISON.md`'s own historical findings** (this file, above) — e.g. the quoted old footer text in the "Discrepancies" section and the "Brand name — flag" section's finding. These are records of what was actually true of the live site / this app at the time each pass ran; rewriting them would falsify the audit trail. Only the "Brand name" resolution-tracker line was marked resolved in place, same pattern as every other resolved item.
- **The `efn-` Tailwind color token prefix** (`efn-green`, `efn-black`, etc. across ~30 files) — these are internal design-token names derived from the domain abbreviation, not visible brand-name text. Renaming them is a pure refactor with no visible effect and wasn't in scope of "copy or metadata"; flagging in case you'd still like them renamed for consistency.
- **The "Encore Nutrition" / "Encore Fitness" section eyebrow labels** on the Home page (`src/pages/Home.tsx`, `src/components/WhyChooseEncore.tsx`) — these are verbatim-scraped section labels from the live site's actual Elementor markup (distinguishing the products section from the fitness section), not the overall business name. They don't contain the phrase "Encore Fitness and Nutrition" and weren't touched by this pass; say the word if you want these reworded too.

## Sixth pass — brochure cross-check (Till number, Weekly Pass, schedule conflict, contact details)

A photo of the physical Encore Fitness Studio brochure, plus a WhatsApp pricing confirmation from staff (Madam Mary K Gachambi, Alley Inn — 2026-08-24), surfaced several data points not previously captured from the live WordPress site (which doesn't publish pricing or a Till number at all). Cross-checked against what's live in this app:

**Applied, purely additive — no conflict:**
- **M-Pesa Till number: 9442965.** Added to `settings` (`mpesa.till_number`), shown in the footer and on the checkout confirmation screen when M-Pesa is the chosen payment method. `supabase/migrations/0009_till_number_weekly_pass.sql`.
- **Weekly Pass, KES 1,500.** Added as a new `membership_plans` row, sorted between Daily and Monthly per the brochure's order. Same migration.
- **Instagram handle corrected to @encorefitnessstudio** (brochure) over `encorefitness_buru` (the handle actually linked from the live WordPress site, scraped in the first pass). Client confirmed the brochure's handle is correct — `src/pages/Contacts.tsx`.
- **Street name "Mumias Road" merged into the address.** The live site (and this app, until now) only had "Suite No. 228" with no street — the brochure adds the street name, purely additive, no conflict. `src/pages/Contacts.tsx` now reads "2nd Floor, Quickmart Buruburu Building, Mumias Road, Suite No. 228, Nairobi".

**Flagged, not applied — asked, resolved:**
